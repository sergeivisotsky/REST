...
/**
 * Test for {@link OrderControllerV2}
 *
...
public class OrderControllerV2Test {
    ...
    private static final String BASE_URL = "http://localhost/api/v2/customers";
    ...
    public void findAllOrders_thenReturnOk() throws Exception {
        ...
        Order savedOrder = setupOrder(customer, orderDate, requiredDate, shippedDate, status);

        mvc.perform(
                get(BASE_URL + "/" + customer.getCustomerId() + ORDER_URI)
                        .contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$._embedded.orderDTOV2List[0].orderId").isNotEmpty())
                .andExpect(jsonPath("$._embedded.orderDTOV2List[0].customerId").isNotEmpty())
                .andExpect(jsonPath("$._embedded.orderDTOV2List[0].orderDate").value("2018-09-28T22:00:00"))
                .andExpect(jsonPath("$._embedded.orderDTOV2List[0].requiredDate").value("2018-09-29T22:00:00"))
                .andExpect(jsonPath("$._embedded.orderDTOV2List[0].shippedDate").value("2018-09-30T22:00:00"))
                .andExpect(jsonPath("$._embedded.orderDTOV2List[0].status").value(status))
                .andExpect(jsonPath("$._embedded.orderDTOV2List[0]._links.self.href",
                        is(BASE_URL + "/" + customer.getCustomerId() + ORDER_URI + "/" + savedOrder.getOrderId())))
                .andExpect(jsonPath("$._embedded.orderDTOV2List[0].orderDetails").isEmpty())
                /*.andExpect(jsonPath("$._embedded.orderDTOV2List[0].orderDetails[0].productCode").value(productCode))
                .andExpect(jsonPath("$._embedded.orderDTOV2List[0].orderDetails[0].quantityOrdered").value(quantityOrdered))
                .andExpect(jsonPath("$._embedded.orderDTOV2List[0].orderDetails[0].price").value(orderPrice))*/
                .andExpect(jsonPath("$._links.self.href",
                        is(BASE_URL + "/" + customer.getCustomerId() + ORDER_URI)))
        ;
    ...
    public void findOneOrder_thenReturnOk() throws Exception {
        ...
        mvc.perform(
                get(BASE_URL + "/" + customer.getCustomerId() + ORDER_URI + "/" + savedOrder.getOrderId())
                        .contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
                .andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$._links.self.href")
                        .value(BASE_URL + "/" + customer.getCustomerId() + ORDER_URI + "/" + savedOrder.getOrderId()))
                .andExpect(jsonPath("$._links.allCustomerOrders.href")
                        .value(BASE_URL + "/" + customer.getCustomerId() + ORDER_URI))
                .andExpect(jsonPath("$.status").value(status))
